...
プロジェクトはmonorepo構成になっているため、下記のようなディレクトリ構造で整備してください。
...
graph LR
  ...
  docs_dir --> project_dir["{project名}/<br><i>プロジェクト固有のドキュメント格納</i>"]
  project_dir --> file_specification["specification.md<br><i>プロジェクト毎の全体仕様書</i>"]
  project_dir --> file_feature_list["feature-list.md<br><i>機能一覧と機能簡易説明</i>"]

  project_dir --> architecture_dir["architecture/<br><i>アーキテクチャ詳細</i>"]
  ...
  docs_dir --> file_readme["README.md<br><i>全体の概要</i>"]
```

### **1. プロジェクト配下（docs/{project名}/ 以下）**
...
- **詳細ルール**:
  ...
  - 下記の項目を必ず含む
    ...
    - データベース：各テーブルの概要と目的・意義を記載する。詳細なER図やデータベース設計は別途docs/{project名}/architecture/db-definition.mdに記載する。
      - データモデル・設計：各テーブルの概要と目的・意義を記載する（docs/{project名}/architecture/db-definition.mdへのリンクを貼る）
      - データの流れ：システム内・外とのデータ連携ポイント、入力元と出力先を整理する。
    - 画面UI・操作フロー：詳細な画面遷移図などはdocs/{project名}/screens配下に記述してください。
      - 画面構成：各画面の概要と目的を記載する。各画面の構成要素や可能な操作一覧の詳細はdocs/{project名}/screens/flow.mdに記載してください。
...
- **path**: 各プロジェクトごとに、`docs/{project名}/specification.md` として作成してください。
...
### 機能一覧
- **path**: `docs/{project名}/feature-list.md`
...
- **path**:
  ...
  - プロジェクト個別アーキテクチャ: `docs/{project名}/architecture/architecture.md`
- **概要**: システムのディレクトリ構成、各ディレクトリの役割、全体のアーキテクチャ設計について記載。

### データベース

- **path**: `docs/{project名}/architecture/db-definition.md`
...
- **path**:
  - 全体インフラ: `docs/architecture/infra.md`
  - プロジェクト個別インフラ: `docs/{project名}/architecture/infra.md`
...
- **path**: 各画面ごとに、`docs/{project名}/screens/` 以下に個別ファイルとして作成してください。
- **概要**: 各画面の構成、遷移フロー、利用シーンについて記載。
- **詳細ルール**:
  - 画面遷移はflow.mdにmermaidで記載
  - 各画面の構成やユースケースは、画面ごとに記載。例: `docs/{project名}/screens/order.md`
...
- **path**: 各エラーごとに、`docs/{project名}/errors/%Y-%m-%d_%H:%M-{error名}.md` として作成してください。
